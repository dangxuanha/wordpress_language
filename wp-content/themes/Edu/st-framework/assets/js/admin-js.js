...
jQuery(document).ready(function() {
    ...
    jQuery('body').on('st-panel-change',function(){

        jQuery('.STpanel-content, STpanel-content .STpanel-tab').css('min-height','');
        jQuery('.STpanel-tabs').css('min-height','');

        var id_height = jQuery('.STpanel-content .tab-active').innerHeight();
        var sidebar_height = jQuery('.STpanel-tabs').innerHeight();
        if(sidebar_height < id_height){
            jQuery('.STpanel-content .tab-active, .STpanel-tabs, .STpanel-content').css('min-height',id_height);
        } else {
            jQuery('.STpanel-content .tab-active, .STpanel-tabs, .STpanel-content').css('min-height',sidebar_height);
        }
    ...
    //--------- show hide combo box --------------
    // supports select and radio input only
    jQuery('.show-on-changes').each(function(){
        var box = jQuery(this);
        var show_when = box.attr('show-when') || '';
        var show_on = box.attr('show-on') || '';

        // when load
        // radio first
        var v ;
        if(jQuery('input.'+show_when+':radio').length){

            v =jQuery('input.'+show_when+':radio:checked').val() || '';
            if(v!==''){
                jQuery('.show-on-changes[show-when="'+show_when+'"][show-on~="'+v+'"]').show();
            }

            jQuery('input.'+show_when+':radio').change(function(){
                jQuery('.show-on-changes[show-when="'+show_when+'"]').hide();
                v =jQuery('input.'+show_when+':radio:checked').val() || '';
                if(v!==''){
                    jQuery('.show-on-changes[show-when="'+show_when+'"][show-on~="'+v+'"]').show();
                }
                jQuery('body').trigger('st-panel-change');
            });

        }else if(jQuery('select.'+show_when).length){

            v =jQuery('select.'+show_when).val() || '';
            if(v!==''){
                jQuery('.show-on-changes[show-when="'+show_when+'"][show-on~="'+v+'"]').show()
            }

            jQuery('select.'+show_when).change(function(){
                jQuery('.show-on-changes[show-when="'+show_when+'"]').hide();
                v =jQuery('select.'+show_when).val() || '';
                if(v!==''){
                    jQuery('.show-on-changes[show-when="'+show_when+'"][show-on~="'+v+'"]').show();
        ...
        var  p = st_getCookie('st_pntab_active_parent');
         ...
         if(typeof(id)!='undefined' && id!=''){
            ...
            if(ftab.find('ul').length){ // had child
                ...
                id = a.attr('href');
                jQuery('.STpanel-tab').removeClass('tab-active').hide();
                jQuery('.STpanel-tab'+id).addClass('tab-active').show();
               // alert(id);
               
            }else{
                 jQuery('.STpanel-tab').eq(0).addClass('tab-active').fadeIn();
    ...
    jQuery('.STpanel-click-tabs .parent li a, .STpanel-click-tabs  li.no-child a').click(function(){
        var li = jQuery(this).parent('li');
         ...
         jQuery('.STpanel-tab').fadeOut();
         jQuery('.STpanel-tab').removeClass('tab-active');
         jQuery(id).fadeIn();
