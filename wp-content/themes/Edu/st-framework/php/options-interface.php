...
require_once(ST_FRAMEWORK_PHP.'theme-options.php');
...
$values = get_option(ST_FRAMEWORK_SETTINGS_OPTION,array());
?>
<div class="wrap">

<div class="icon32" id="icon-options-general"><br/></div>
<h2><?php _e('Option Settings', 'smooththemes'); ?></h2>

<?php
if(function_exists('st_is_wpml') && st_is_wpml()){ // if WPML installed
    $current_tran_lang = ICL_LANGUAGE_CODE;
    $langs = icl_get_languages('skip_missing=0&orderby=code&order=asc');

    // get current lang
    foreach($langs as $l){
        if( $current_tran_lang == $l['language_code'] ){
            $current_lang = $l;
        }
    ...
    if($_POST['st_save_lang']=='y'){
        ...
        if($_POST['st_lang_copy_from']!='' && $_POST['st_lang_copy_to']!=''){
            ...
            if($l_from='st_default'){ // copy from default settings
                $l_options  =  get_option(ST_FRAMEWORK_SETTINGS_OPTION,array()) ;
            }else{
                $l_options  =  get_option(ST_FRAMEWORK_SETTINGS_OPTION.'_'.$l_from,array()) ;
            }

            update_option(ST_FRAMEWORK_SETTINGS_OPTION.'_'.$l_to,$l_options);
        }
    ...
    <form action="" method="post">
        ...
        <p>
            <label><input type="checkbox" id="st_lang_same_settings" name="st_lang_same_settings" value="y" <?php echo $st_same_settings =='y' ? ' checked = "checked" ' : ''; ?> />
                <?php _e('Keep same settings for all languages','smooththemes') ?>
            </label>
        </p>
    ...
    if($st_same_settings=='y'){
        ...
        // ICL_LANGUAGE_CODE
        $values = get_option(ST_FRAMEWORK_SETTINGS_OPTION.'_'.ICL_LANGUAGE_CODE,array());
        if(empty($values)){
            $values = get_option(ST_FRAMEWORK_SETTINGS_OPTION,array());  // default value
        }
    }

    if($st_same_settings!='y'){

        $st_tran_page = ST_FRAMEWORK_PAGE_SLUG;
        ?>
...
<?php if(isset($_POST['save']) && $_POST['save'] == 'Y'): ?>
    <div class="updated" id="message-" style="margin: 30px 0px 0px;">
        <p><strong><?php echo ucfirst(ST_THEME_NAME); ?> theme options updated.</strong></p>
    </div>
<?php endif; ?>
...
<?php if(isset($_REQUEST['to_default']) && $_REQUEST['to_default'] == 1): ?>
    <div class="updated" id="message-" style="margin: 30px 0px 0px;">
        <p><strong><?php echo ucfirst(ST_THEME_NAME); ?> <?php _e('theme options has been RESET.','smooththemes'); ?></strong></p>
    </div>
<?php endif; ?>


<?php if($_REQUEST['export']=='true'){
    ?>
    <div class="export" style="margin: 20px 0px;">
        <?php// echo var_dump($current_lang); ?>
        <?php
        //if(!empty($current_lang)){
        //   printf('Export code for %s','<a')
        //}
        ?>
        <b>Export Code:</b><br/>
        <textarea style="height: 300px; width: 100%;" readonly="readonly"><?php echo base64_encode(maybe_serialize($values)); ?></textarea>
    </div>
<?php
}
...
<form class="st_options_form" action="admin.php?page=<?php echo $_REQUEST['page']; echo (defined('ICL_LANGUAGE_CODE')) ? '&lang='.ICL_LANGUAGE_CODE : ''; ?>" method="post"  enctype="multipart/form-data" >
    ...
    <div class="STpanel-wrap">
        ...
        <div class="STpanel-header">
            <?php if (st_get_setting('st_framework', 'y') == 'y') { ?>
                <div class="STpanel-h-info">

                    <div class="STpanel-header-left">
                        <a title="" href="http://www.smooththemes.com/" target="_blank"><img src="<?php echo ST_FRAMEWORK_IMG .'logo.jpg' ?>" alt="" title="" /></a>
                    </div>
            ...
            <div class="STpanel-tabs">
                ...
                <ul class="STpanel-click-tabs">
                    <?php foreach($tabs_settings->tabs as $tab):
                        ...
                        if(isset($tab['child']) && count($tab['child'])){
                            $class ='parent';
                        }else{
                        ...
                        <li class="<?php echo $class; ?> lv-1 tab-<?php echo esc_attr($tab['tab_id']); ?>">
                            ...
                            <?php if(isset($tab['child']) && count($tab['child'])): ?>
