...
class  Admin_Tabs_Display {
    var $code            = '';
    var $name            = ST_FRAMEWORK_SETTINGS_OPTION;
    ...
    function display_tab_contents($fields=array()){
        ...
        foreach($fields as $field){
            ...
            if(isset($field['show_on'])  && $field['show_on']!='' && isset($field['show_when']) && $field['show_when']!=''){
                echo '<div class="show-on-changes" show-when="'.esc_attr($this->name.'-'.$field['show_when']).'" show-on="'.esc_attr($field['show_on']).'">';
    ...
    */
    ...
    function  box_content_inner($input){
       ...
       if(isset($this->field['desc']) && $this->field['desc']!=''){
         ...
         if(isset($this->field['desc_bottom']) && $this->field['desc_bottom']!=''){
    ...
    */
    ...
    function editor(){
        ...
        wp_editor($this->value(3),$this->name());
    ...
    */
    function upload(){
        echo  '<div class="STpanel-input-box box-upload">';
         ...
         $input_type ='text';
          if(isset($this->field['value_type']) && strtolower($this->field['value_type'])=='id'){
            $input_type ='hidden';
         }
    ...
    function  options_selected($option_value){
        ...
        if($values=='' && isset($this->field['default'])){
             $values = $this->field['default'];
        }
    ...
    function select(){
        ...
        if(isset($this->field['multiple']) && $this->field['multiple']){
            ...
            $input = '<select class="chzn-select bp-input-select '.esc_attr($this->name.'-'.$this->field['name']).'" name="'.$name.'">';
    ...
    function radio(){
         ...
         foreach($this->field['options'] as $k => $label){
           $input.='<div class="STpanel-radio-div-item">';
             $input.='<label class="layout-radio"><input value="'.htmlspecialchars($k).'" class="STpanel-radio-input '.esc_attr($this->name.'-'.$this->field['name']).'" type="radio" '.$this->radio_checked($k).' name="'.$this->name().'" />'.$label.'</label>';
             $input.='</div>';
         }
    ...
    function layout(){
         ...
         if(isset($this->field['size']) && $this->field['size']!=''){
    ...
    function  style(){
         ...
         if(isset($support_font_size) && $support_font_size){
         $input='
         ...
         // for line height
         if(isset($support_line_height) && $support_line_height){
           $input .='
        ...
        $input .='<div class="style-inline">';
         if(isset($support_color) && $support_color){
        ...
        // for google font
        if(isset($support_font_family) && $support_font_family){
         ...
         // for font style 
         if(isset($support_font_style) && $support_font_style){
         ...
         // for font Wieght 
         if(isset($support_font_weight) && $support_font_weight){
     ...
     */ 
        ...
        $name =$this->name();
        $values = array();
        if (isset($this->values[$this->field['name']]))
            $values = $this->values[$this->field['name']];
